...
## 5. Addendum — auditoria final e fix loop

| ID  | Sev         | Achado                                                                                                                                                                                     | Estado                                                                                                                                                                                                                                                                                                                |
| --- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| X-1 | CRITICAL    | standalone e Electron `resources/app` empacotavam o `.env` real do checkout, `.git`, `tests/`, `.npmrc`, `.install-upgrade/` (tracing emite a raiz inteira; excludes não casam no Windows) | CORRIGIDO `f7d4acb0f` + `d8fc212b6`/`29c18ac6c` + `5610be10d` (`STANDALONE_PRUNE_TARGETS`, prune fatal, gate `check:standalone-hygiene`) + `d1d2155b6`/`f2b3f7b18` (excludes só para dist dirs irmãos; nunca o próprio) + gate `check:standalone-boot` (`367f4ce31`); bundle real verificado (`TEST_MATRIX.md` §3/§5) |
| X-2 | HIGH        | cache semântico servia resposta de um formato/contexto de cliente a outro                                                                                                                  | CORRIGIDO `f5f7bb387` + `2855c6a6e` (formato + contexto na assinatura; sem HIT JSON para stream não-openai)                                                                                                                                                                                                           |
| B-1 | LOW         | `isPrivateHost` deixava passar IPv6 mapeado não comprimido / NAT64                                                                                                                         | CORRIGIDO `7dcfcfa83` (guard canônico e cópia do relay; 127/127 com probe ofensivo do Auditor B)                                                                                                                                                                                                                      |
| B-2 | IMPROVEMENT | plugins sem assinatura de publisher                                                                                                                                                        | ACEITO — próximo passo minisign/cosign                                                                                                                                                                                                                                                                                |
| B-3 | INFO        | chave cifrada-recuperável para o OpenAPI Try                                                                                                                                               | ACEITO/documentado                                                                                                                                                                                                                                                                                                    |

Gates de segurança do release (estado final): `check:standalone-hygiene` PASS no standalone do build final e no `resources/app` do Electron; `check:standalone-boot` PASS (perfil de produção com `STORAGE_ENCRYPTION_KEY`); `npm audit --omit=dev` 0 crit / 0 high; secret sweep em todo commit (gitleaks/semgrep no CI).
