...
## 9. Adendo — publicação (2026-09-12)

Dentro da autorização condicional do operador: PR #5 mesclado em `release/v3.8.51` (`bd518dafd`, 18/18 checks verdes após 4 rodadas de correção no CI Linux); PR #6 (`ce776db34`) tornou o `docker-publish.yml` tolerante à ausência de credenciais do Docker Hub (o fork não as tem, e as duas runs anteriores morriam no login); a run 34682777453 publicou `ghcr.io/lmprado-dz23/omniroute:next` (índice OCI `sha256:ce399d87…`, linux/amd64 + linux/arm64) e `:next-web` (`sha256:07417c57…`), com SBOM CycloneDX e gate Trivy CRITICAL aprovado; pull anônimo verificado. A tag de versão `:3.8.51`/`:latest` depende de uma tag git `v3.8.51` ou Release (dispara também o Electron release) — decisão do operador. Não executados, por estarem fora da autorização: npm publish, Docker Hub, GitHub Release, deploy.
