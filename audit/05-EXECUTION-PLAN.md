...
- **Fase 2:** R-1 rollback = restaurar snapshot pré-migração com verificação (+ `ROLLBACK.md`); R-2 transações (`createApiKey`, `deleteApiKey`, `reorderConnections`, `registeredKeys`, `callLogs`); R-3 retry `SQLITE_BUSY`; R-4 idempotência honesta; R-5 guard "bytes emitidos ⇒ sem refazer"; R-6 shutdown único + `globalThis` guards; R-12 `detached`/PID; R-13; R-14; R-20 `ignoreBuildErrors` → progressivo (open-sse já 0; dashboard/api por baseline; `electron` `@ts-check`; `bin` tsconfig). **Números de lint/format/`any`/`@ts-ignore`: a confirmar pelo auditor de qualidade.**
